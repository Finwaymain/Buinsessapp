...
class ServicePriceLineItem {
  ...
  String get displayPrice {
    final sym = (Constant.currency != null && Constant.currency!.trim().isNotEmpty) ? Constant.currency!.trim() : '₹';
    ...
    if (maxPrice > minPrice && (maxPrice - minPrice).abs() >= 1) {
      return '$sym${minPrice.toStringAsFixed(0)}-$sym${maxPrice.toStringAsFixed(0)}';
    }
    if (minPrice > 0) return '$sym${minPrice.toStringAsFixed(0)}';
...
class ServicePriceEstimate {
  ...
  String get effectiveCurrency {
    if (currencySymbol.trim().isNotEmpty) return currencySymbol.trim();
    if (Constant.currency != null && Constant.currency!.trim().isNotEmpty) return Constant.currency!.trim();
    return '₹';
  }

  String get displayTotal {
    final sym = effectiveCurrency;
    if (totalLabel.isNotEmpty) return totalLabel;
    if (totalMax > totalMin && (totalMax - totalMin).abs() >= 1 && totalMin > 0) {
      return '$sym${totalMin.toStringAsFixed(0)}-$sym${totalMax.toStringAsFixed(0)}';
    }
    if (totalMin > 0) return '$sym${totalMin.toStringAsFixed(0)}';
    return 'Rate on visit';
  }
  ...
  String get displayVisitingCharge {
    ...
    if (visitingChargeMax > visitingChargeMin && (visitingChargeMax - visitingChargeMin).abs() >= 1 && visitingChargeMin > 0) {
      return '$sym${visitingChargeMin.toStringAsFixed(0)}-$sym${visitingChargeMax.toStringAsFixed(0)}';
    }
    if (visitingChargeMin > 0) return '$sym${visitingChargeMin.toStringAsFixed(0)}';
    return '';
  }
