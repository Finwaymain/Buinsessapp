...
class LocationPickerHelper {
  ...
  static Future<bool> ensureLocationAccess({BuildContext? context, bool showPromptDialog = true}) async {
    try {
      ...
      if (permission == LocationPermission.denied) {
        ShowToastDialog.showToast('Location permission is required for driver navigation & rides.'.tr);
        return false;
      }
  ...
  static Future<bool> _showEnableGpsDialog(BuildContext? context) async {
    ...
    final result = await showDialog<bool>(
      ...
      builder: (dialogCtx) => AlertDialog(
        ...
        content: Text(
          'Your device location is turned off. Please turn on GPS so you can receive rides and track trip routes.'.tr,
  ...
  static Future<bool> _showPermissionSettingsDialog(BuildContext? context) async {
    ...
    final result = await showDialog<bool>(
      ...
      builder: (dialogCtx) => AlertDialog(
        ...
        content: Text(
          'Location permission is permanently denied. Please grant location access in App Settings to go online and receive bookings.'.tr,
          style: const TextStyle(fontSize: 14, color: Colors.black87),
        ),
        actions: [
          TextButton(
  ...
  static Future<String> _resolveAddress(double latitude, double longitude) async {
    ...
    try {
      final package = Platform.isAndroid ? 'com.fiinwaybusiness' : 'com.fiinwaybusiness.ios';
      final url =
