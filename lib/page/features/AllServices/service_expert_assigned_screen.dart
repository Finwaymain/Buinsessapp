...
import 'package:cabme_driver/page/features/AllServices/service_category_icon.dart';
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Price Breakup'.tr, style: const TextStyle(fontFamily: AppThemeData.bold, fontSize: 18)),
            const SizedBox(height: 12),
            ...breakdown.serviceItems.map((item) {
              if (booking.hasPromotionalBonus && breakdown.serviceItems.indexOf(item) == 0) {
                final visit = breakdown.visitingCharge;
                final curPrice = item.priceAvailable ? item.minPrice : (double.tryParse(item.price.toString()) ?? 0);
                if (curPrice + visit < booking.displayedBookingTotal && booking.promotionalAmountValue > 0) {
                  ...
                  return _breakupRow(item.name, '${Constant.currency ?? '₹'}${boosted.toStringAsFixed(0)}');
                }
              }
              return _breakupRow(item.name, item.displayPrice);
            }),
            ...
            children: [
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(booking.hasPromotionalBonus ? 'Total Payable (Base)'.tr : 'Total Payable'.tr, style: const TextStyle(fontFamily: AppThemeData.bold, fontSize: 14)),
