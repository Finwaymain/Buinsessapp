...
class _ServiceConfirmBookingScreenState extends State<ServiceConfirmBookingScreen> {
  ...
  void _refreshTimeSlots({bool resetSelection = false}) {
    ...
    if (_selectedDateIndex >= _dateOptions.length) {
  ...
  String _readUserPhone() {
    try {
      ...
      if (map['data'] != null) {
        ...
        return userModel.userData?.phone ?? userModel.userData?.alternatePhone ?? '';
      }
  ...
  List<_DateOption> _buildDateOptions() {
    ...
    // Check next 7 days and only include days that have available slots
    for (int i = 0; validOptions.length < 4 && i < 7; i++) {
      final date = DateTime(now.year, now.month, now.day + i);
      final slots = _slotsForDate(date);

      // Hide day completely if no time slots are available
  ...
  Future<void> _loadEstimate() async {
    final estimate = await _controller.fetchPriceEstimate(
      ...
      serviceNames: widget.draft.selectedServices,
      lat: _lat,
      lng: _lng,
    );
    if (mounted) {
  ...
  String get _selectedDateStr {
    if (_dateOptions.isEmpty) return '';
  ...
  Future<void> _confirmBooking(Color btnColor) async {
    ...
    final bookingId = await _controller.bookService({
      ...
      'amount': _estimate!.payableAmountFor(includeServicePrices: _hasSelectedSubServices),
                            ...
                            children: [
                              ...
                              if (_dateOptions.isEmpty)
                                Padding(
                                  ...
                                  child: Text(
                                    'No booking slots available at this moment. Please check back later.'.tr,
                                    style: TextStyle(fontSize: 12, color: _mutedColor(isDarkMode)),
                                  ),
                                )
                              else ...[
