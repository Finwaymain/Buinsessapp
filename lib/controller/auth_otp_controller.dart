...
class AuthOtpController extends GetxController {
  ...
  Future<bool?> checkUserExists(String phoneNumber, {String userCat = 'driver'}) async {
    try {
      isLoading.value = true;
      final body = jsonEncode({
        'phone': phoneNumber,
  ...
  // ── Login by MPIN ──────────────────────────────────────────────────────────
  Future<UserModel?> loginByMpin(String phoneNumber, String mpin, {String userCat = 'driver'}) async {
    try {
      isLoading.value = true;
      final body = jsonEncode({
        'phone': phoneNumber,
  ...
  // ── Reset MPIN ─────────────────────────────────────────────────────────────
  Future<bool> resetMpin(String phoneNumber, String otp, String mpin, {String userCat = 'driver'}) async {
    try {
      isLoading.value = true;
      final body = jsonEncode({
        'phone': phoneNumber,
  ...
  Future<bool> applyReferralCode(String userId, String referralCode, {String userCat = 'driver'}) async {
    try {
      isLoading.value = true;
      final body = jsonEncode({
        'user_id': userId,
  ...
  Future<UserModel> _saveAndReturnUser(Map<String, dynamic> responseBody) async {
    ...
    if (model.userData != null) {
      await Preferences.setInt(Preferences.userId, int.parse(model.userData!.id.toString()));
  ...
  Future<UserModel?> updateDriverCategory(String categoryId, List<String> subcategoryIds) async {
    try {
      isLoading.value = true;
      final body = jsonEncode({
      ...
      if (res.statusCode == 200 && data['success'] == 'success') {
        return _saveAndReturnUser(data);
      } else {
        ShowToastDialog.showToast(data['error'] ?? 'Failed to update category');
        return null;
      }
    } catch (e) {
      isLoading.value = false;
