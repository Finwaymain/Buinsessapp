...
class SearchAddressController extends GetxController {
  ...
  Future<SearchInfo?> resolvePlaceDetails(SearchInfo suggestion) async {
    ...
    try {
      ...
      log("Error resolving place details: $e");
    }
    return null;
  }
  ...
  Future<void> fetchAddress(String text) async {
    ...
    log("fetchAddress => text: $text");
    isSearch.value = true;
    try {
      if (Constant.selectedMapType == 'google') {
        log("Using Google Places Autocomplete API...");
          ...
          if (decoded != null && decoded['predictions'] != null) {
            ...
            for (var item in predictions) {
              try {
                ...
                log("Error parsing google prediction: $e");
              }
            }
      ...
      // Fallback 1: Query Photon API directly
      log("Fallback 1: Querying Photon API directly...");
      ...
      final photonResponse = await http.get(photonUri, headers: {
        'User-Agent': Platform.isAndroid ? 'com.cabme.driver' : 'com.cabme.driver.ios',
      });
        ...
        if (decoded != null && decoded['features'] != null) {
          ...
          for (var item in features) {
            try {
              ...
              log("Error parsing photon item: $parseError");
            }
          }
          if (results.isNotEmpty) {
            suggestionsList.value = results;
      ...
      // Fallback 2: Query Nominatim Search API directly
      log("Fallback 2: Querying Nominatim Search API directly...");
      ...
      if (nominatimResponse.statusCode == 200) {
        ...
        for (var item in decoded) {
          try {
            ...
            log("Error parsing nominatim item: $parseError");
          }
        }
        if (results.isNotEmpty) {
          suggestionsList.value = results;
      ...
      log("fetchAddress => error: ${e.toString()}");
      suggestionsList.clear();
    } finally {
      isSearch.value = false;
    }
